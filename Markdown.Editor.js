...
(function () {
    ...
    var defaultsStrings = {
        ...
        hr: "Горизонтальная линия <hr> Ctrl+R",

        undo: "Undo - Ctrl+Z",
        redo: "Redo - Ctrl+Y",
        redomac: "Redo - Ctrl+Shift+Z",

        help: "Markdown помощь"
    };
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, getString) {
        ...
        function makeSpritedButtonRow() {
            ...
            buttons.bold = makeButton("icon-bold", getString("bold"), "0px", bindCommand("doBold"));
            buttons.italic = makeButton("icon-italic", getString("italic"), "-20px", bindCommand("doItalic"));
            makeSpacer(1);
            buttons.link = makeButton("icon-link", getString("link"), "-40px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, false);
            }));
            buttons.quote = makeButton("icon-quotes-left", getString("quote"), "-60px", bindCommand("doBlockquote"));
            buttons.code = makeButton("icon-embed", getString("code"), "-80px", bindCommand("doCode"));
           /* buttons.image = makeButton("icon-image", getString("image"), "-100px", bindCommand(function (chunk, postProcessing) {
                return this.doLinkOrImage(chunk, postProcessing, true);
            })); */
            makeSpacer(2);
            buttons.olist = makeButton("icon-list-numbered", getString("olist"), "-120px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, true);
            }));
            buttons.ulist = makeButton("icon-list2", getString("ulist"), "-140px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, false);
            }));
            buttons.heading = makeButton("icon-font-size", getString("heading"), "-160px", bindCommand("doHeading"));
            buttons.hr = makeButton("icon-page-break", getString("hr"), "-180px", bindCommand("doHorizontalRule"));
            makeSpacer(3);
            buttons.undo = makeButton("icon-undo", getString("undo"), "-200px", null);
            ...
            buttons.redo = makeButton("icon-redo", redoTitle, "-220px", null);
